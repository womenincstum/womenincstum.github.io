...
function verifyCode(email, userCode, name, gender, status, field, phone) {
  if (!email || !userCode) {
  ...
  for (var i = data.length - 1; i >= 0; i--) {
    ...
    if (rowEmail === incomingEmail && rowCode === incomingCode) {
      ...
      if (!members) {
        ...
        members.appendRow(["Email", "Name", "Gender", "Study Status", "Study Program", "WhatsApp", "Joined At"]);
      }
      members.appendRow([email, name, gender, status, field, phone, new Date()]);
